...
const Vector &NonlinearForm::mProlongate(const Vector &x) const
{
   MFEM_VERIFY(x.Size() == mfes->GetTrueVSize(), "invalid input (mixed) Vector size");
   if (mP)
   {
      maux.SetSize(mP->Height());
      mP->Mult(x, maux);
      return maux;
   }
   return x;
...
const Vector &NonlinearForm::Prolongate_old(const Vector &x) const
{
   MFEM_VERIFY(x.Size() == fes_old->GetTrueVSize(), "invalid input (old states) Vector size");
   if (P_old)
   {
      aux_old.SetSize(P_old->Height());
      P_old->Mult(x, aux_old);
      return aux_old;
   }
   return x;
}
...
void NonlinearForm::Mult(const Vector &x, const Vector &mx, Vector &y) const
{
   const Vector &px = Prolongate(x);
      ...
      for (int i = 0; i < fes -> GetNBE(); i++)
      {
         ...
         if (tr != NULL)
         {
            ...
            for (int k = 0; k < bfnfi.Size(); k++)
            {
               ...
               bfnfi[k]->AssembleFaceVector(*fe1, *fe2, *tr, el_x, el_mx, el_y);
               py.AddElementVector(vdofs, el_y);
            }
         }
      }
...
void NonlinearForm::Mult(const Vector &x, const Vector &mx, const Vector &x_old, Vector &y) const
{
   const Vector &px = Prolongate(x);
   const Vector &mpx= mProlongate(mx);
   ...
   Array<int> vdofs, mvdofs, vdofs_old;
   ...
   if (fnfi.Size())
   {
      FaceElementTransformations *tr;
      const FiniteElement *fe1, *fe2;
      Array<int> vdofs2, mvdofs2, vdofs2_old;
